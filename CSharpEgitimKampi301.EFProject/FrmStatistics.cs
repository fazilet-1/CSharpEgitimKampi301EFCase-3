...
namespace CSharpEgitimKampi301.EFProject
{
    public partial class FrmStatistics : Form
    {
        ...
        { 
            ...
            lblAvgCapacity.Text = db.Location.Average(x => (decimal?)x.Capacity)?.ToString("0.00").ToString();
            ...
            lblCappadociaLocationCapacity.Text = db.Location.Where(x => x.City == "Kapadokya")
                .Select(y => y.Capacity).FirstOrDefault().ToString();

            lblTurkiyeCapacityAvg.Text = db.Location.Where(x => x.Country == "Türkiye").Average(y => y.Capacity)
                .ToString();

            var romeGuideId = db.Location.Where(x => x.City == "Roma Turistik")
            ...
            lblMaxCapaciytLocation.Text = db.Location.Where(x => x.Capacity == maxCapacity).Select(y => y.City)
                .FirstOrDefault().ToString();
            ...
            var guideIdByNameAysegulCınar = db.Guide.Where(x => x.GuideName == "Ayşegül" && x.GuideSurname == "Çınar").Select(y => y.GuideId)
                .FirstOrDefault();
